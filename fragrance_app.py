...
if "fragrances_db" not in st.session_state:
    ...
    _seed_db = list(SANCTUARY_SEED_DB)
...
with tab_vault:
    ...
    with st.expander("Backup & restore", expanded=True):
        _seed_n = len(SANCTUARY_SEED_DB or [])
        if st.button(
            f"Restore full sanctuary seed ({_seed_n} bottles)",
            key="restore_sanctuary_seed",
        ):
            st.session_state["fragrances_db"] = list(SANCTUARY_SEED_DB)
            mark_vault_dirty()
            save_persisted_data(force=True)
            st.success(f"Restored **{_seed_n}** sanctuary bottles. Export JSON and keep a copy.")
            st.rerun()
