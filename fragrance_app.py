...
def suggest_partners_for(
    ...
    for f in pool:
        ...
        cats_b = ", ".join((base.get("category") or [])[:3]) or "ÃÂ¢ÃÂÃÂ"
        cats_p = ", ".join((f.get("category") or [])[:3]) or "ÃÂ¢ÃÂÃÂ"
...
with tab_discover:
    ...
    with st.expander("Top 5 by season", expanded=True):
        st.caption("Pick a gender, then see your top 5 for Spring, Summer, Fall, and Winter.")
        ts_gender = st.selectbox(
            "Gender for season top 5",
        ...
        if st.button("Show top 5 for each season", type="primary", key="top_season_go"):
            st.session_state["_top_season_results"] = {
                label: top_picks_for_calendar_season(key, gender=ts_gender, top_n=5)
                for label, key in seasons
            }
            ...
            for label, _key in seasons:
                ...
                if not picks:
                    ...
                    for i, f in enumerate(picks, 1):
                        ...
                        c1, c2 = st.columns(2)
                        with c1:
                            if st.button("Log SOTD", key=f"ts_sotd_{label}_{i}_{f.get('name')}"):
                                try:
                                    log_sotd_immediate([f.get("name")], notes=f"Top 5 {label}")
                                except Exception:
                                    send_to_sotd([f.get("name")])
                                st.rerun()
                        with c2:
                            if st.button("To Layer", key=f"ts_layer_{label}_{i}_{f.get('name')}"):
                                _lab = f"{f.get('name')} - {(f.get('brand') or '?').strip()}"
                                st.session_state["layer_base_select"] = _lab
                                st.session_state["_layer_studio_flash"] = (
                                    f"Base set to **{f.get('name')}** - open Layer tab."
                                )
                                st.rerun()
                st.markdown("---")
...
with tab_layer:
    ...
    if ev:
        ...
        if _spray:
            st.caption("Spray order: " + " ÃÂ¢ÃÂÃÂ ".join(_spray))
        if ev.get("why"):
