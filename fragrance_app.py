#!/usr/bin/env python3
"""
Fragrance Generator + Layering Combo – Streamlit App
Run locally with:  streamlit run fragrance_app.py
"""
...
# ====================== PAGE CONFIG ======================
st.set_page_config(
    page_title="Fragrance Generator",
    page_icon="🧴",
    layout="centered",
...
def suggest_layering_combos(pool: List[Dict], num_combos: int = 3) -> List[Tuple[Dict, Dict, str]]:
    ...
    for s, f1, f2, reason in candidates:
        if f1["name"] in used and f2["name"] in used:
            continue
...
st.title("🧴 Fragrance Generator")
st.caption("Get ranked recommendations + layering combos from your collection")
...
with st.sidebar:
    ...
    gender = st.selectbox(
        ...
        index=0,
        help="Male/Female also include Unisex and leaning fragrances"
    )
    ...
    top_n = st.radio("Number of recommendations", [1, 3, 5], index=1, horizontal=True)

    generate = st.button("✨ Generate Recommendations", type="primary", use_container_width=True)

if generate:
    selected = get_top_fragrances(gender, weather, category, occasion, top_n)

    if not selected:
        st.warning("No fragrances matched your filters. Try selecting **Any** for some options.")
    else:
        st.subheader(f"Top {len(selected)} Pick{'s' if len(selected) > 1 else ''} of the Day")

        for i, f in enumerate(selected, 1):
            with st.container(border=True):
                if i == 1:
                    st.markdown(f"### ★ #{i}  {f['name']} – {f['brand']}")
                else:
                    st.markdown(f"### #{i}  {f['name']} – {f['brand']}")
        ...
        if combos:
            st.divider()
            st.subheader("Layering Combos")
            st.caption("Spray the richer/heavier one first, then the lighter one on top.")
            ...
            for i, (f1, f2, reason) in enumerate(combos, 1):
                with st.container(border=True):
                    ...
                    with col1:
                        st.markdown(f"**Base / First**  \n{f1['name']}  \n*{f1['brand']}*")
                    with col2:
                        st.markdown(f"**Layer / Top**  \n{f2['name']}  \n*{f2['brand']}*")
                    st.caption(f"Why it works: {reason}")
else:
    st.info("← Choose your filters in the sidebar, then click **Generate Recommendations**")
    st.write(f"Database loaded: **{len(FRAGRANCES)}** fragrances")
